...
class ALIF(Cell):
    ...
    def __call__(self, inputs, state, scope=None, dtype=tf.float32):
        with tf.name_scope('ALIFcall'):
            ...
            # current 
            I1_nosyn = tf.matmul(inputs, self.W_in) + self.B_in #current generated by input stimuli
            I1_syn = state.I1*tf.exp(-1.0/self.tau_recsyn) + tf.matmul(inputs, self.W_in)*(1-tf.exp(-1.0/self.tau_recsyn)) + self.B_in
            I1 = I1_nosyn #tf.where(tf.greater(self.flag, 0.5), I1_syn, I1_nosyn) #current generated by input stimuli
