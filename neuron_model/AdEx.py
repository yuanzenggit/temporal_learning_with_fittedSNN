...
class ALIF(Cell):
    ...
    def __call__(self, inputs, state, scope=None, dtype=tf.float32):
        with tf.name_scope('ALIFcall'):
            ...
            #I1 = state.I1*tf.exp(-1.0/self.tau_recsyn) + tf.matmul(inputs, self.W_in)*(1-tf.exp(-1.0/self.tau_recsyn)) + self.B_in
            I2 = state.I2*tf.exp(-1.0/self.tau_recsyn) + tf.matmul(state.s, self.W_rec)*(1-tf.exp(-1.0/self.tau_recsyn)) #synapse currrent
            w = state.w*(1 - 1.0/self.tau_w) + self.a*state.s
            
            # membrane potential 
            u = state.u + (-1*(state.u-tf.ones_like(state.u)*self.Vrest) \
                           + self.delT*tf.exp((state.u-tf.ones_like(state.u)*self.Vth)/self.delT) \
            ...
            u = tf.where(tf.greater(u, self.Vmax), tf.ones_like(u)*self.Vmax+1, u) #set voltage upper limit
            u = tf.where(tf.greater(self.Vrest, u), tf.ones_like(u)*self.Vrest, u) #set voltage lower 
            
            # generate spike 
            u_scale = (u-self.Vth)/abs(self.Vth) #normalized membrane different, used for backpropogation
            s = SpikeFunction( u-self.Vmax, u_scale, self.dampening_factor)
            
            # refractory period                                                                              
            ref = tf.where(tf.greater(state.ref, 0.0), tf.ones_like(u)*(state.ref-1), state.ref) #refractory timing reduce 1 step
            ref = tf.where(tf.greater(u, self.Vmax), tf.ones_like(u)*self.t_ref, ref) #enter refractory period
